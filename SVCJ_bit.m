...
load BitDaily
bitP=bitcoindaily.ClosePrice;
TS=datenum(bitcoindaily.Date,'dd/mm/yyyy');


%dadd=693960;
%Mdate = x2mdate(data(:,3), 0)
%dateS=TS+dadd;
dateS=TS;
dateD=year(dateS)*10000+month(dateS)*100+day(dateS);
%GoldBullion(:,1)=dateD;

CrixDATA=[dateS bitP dateD];
idxSt=find(CrixDATA(:,3)==20140731,1,'first');
idxEn=find(CrixDATA(:,3)==20170930,1,'first'); 
CrixData=CrixDATA(idxSt:idxEn,:);
...
n =3.e3; % Specifies the burn-in period
